...
class FriendsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NSLayoutConstraint.activate([
            letterControl.centerYAnchor.constraint(equalTo: friendsTable.centerYAnchor, constant: 0),
            letterControl.trailingAnchor.constraint(equalTo: friendsTable.trailingAnchor, constant: 0),
    ...
    @objc func selectedLetter(){
        ...
        for indexSextion in 0..<firstLetter.count{
            if selectLetter == firstLetter[indexSextion]{
                friendsTable.scrollToRow(at: IndexPath(row: 0, section: indexSextion), at: .top, animated: true)
                break
            }
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showLibrary" {
            ...
            if let indexPath = friendsTable.indexPathForSelectedRow {
                ...
                var user: Users
                if isFiltering {
                    user = filteredFriend[indexPath.row]
                } else {
                    user = arrFriend![indexPath.row]
                }
...
extension FriendsViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.customView.setImage(friend.userPhoto)
        return cell
    }
